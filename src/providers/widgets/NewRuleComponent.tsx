...
export const NewRuleComponent: React.FC<{ ruleAdded: () => {} }> = ({
  ruleAdded,
}) => {
  ...
  const createRule = async () => {
    ...
    if (materialEntry.length > 0) {
      const rule = await ValidationLink.createPipeValidationRule(
        insulationLow,
        insulationHigh,
        tempLow,
        tempHigh,
  ...
  return (
    <>
      ...
      <div style={{ marginTop: "20px" }}>
        Temperature Range:
        <br />
        {getInput(32, 500, 0.5, "Low", (event: any) => {
          setTempLow(Number.parseFloat(event.target.value));
        })}
        {getInput(32, 500, 0.5, "High", (event: any) => {
      ...
      <div>
        ...
        <br />
        {getInput(0, 4, 0.1, "Low", (event: any) => {
          setInsulationLow(Number.parseFloat(event.target.value));
        })}
        {getInput(0, 4, 0.1, "High", (event: any) => {
          setInsulationHigh(Number.parseFloat(event.target.value));
        })}
      </div>
      <Button
