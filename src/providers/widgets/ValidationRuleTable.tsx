...
export const ValidationRuleTable: React.FC<{
  ruleList: any[];
  rulesSelected: (ruleIds: string[]) => void;
}> = ({ ruleList, rulesSelected }) => {
  ...
  React.useEffect(() => {
    ...
    for (const rule of ruleList) {
      ...
      if (ruleData.material?.label) material = ruleData.material.label;

      data.push({
        material,
        temperature: `${ruleData.tempLow} °C  - ${ruleData.tempHigh} °C`,
        insulation: `${ruleData.insulationLow} m - ${ruleData.insulationHigh} m`,
      });
    }
